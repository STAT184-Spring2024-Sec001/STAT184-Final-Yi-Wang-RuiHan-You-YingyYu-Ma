...
```{r}
# filter factors we want to use for further analysis
filtered_production_method <- TopProductionMethods %>% 
  select(MOVIES, TOTAL.GROSS, MARKET.SHARE)
```


 | STAT          | Min     |1st Qu.  | Median  |Mean      |3rd Qu.   |Max.       |
 |:-------------:|:-------:|:-------:|:-------:|:--------:|:--------:|:---------:|
 | Movies        | 4.0     | 31.5    | 164.0   | 2210.4   | 314.5    | 14613.0   |
 | Average.Gross | 1681858 | 7205034 | 18051814| 33273046 | 41909129 | 114949327 |
 | Market.Share  | 0.00    | 0.15    | 1.25    | 14.28    | 11.41    | 75.56     |
 
## Data Visualization
 
```{r}
# change movie factor to quantitative variable
TopProductionMethods$MOVIES_factor <- factor(TopProductionMethods$MOVIES)
TopProductionMethods$MOVIES_numeric <- as.numeric(TopProductionMethods$MOVIES_factor)

# use ggplot to create bar chart 1
ggplot(TopProductionMethods) +
  aes(x = PRODUCTION.METHODS, weight = MOVIES_numeric) +
  geom_bar(fill = "#112446") +
  labs(title = "Production Methods vs. Movies") +
  theme_minimal() 

# use ggplot to create bar chart 2
ggplot(TopProductionMethods) +
  aes(x = PRODUCTION.METHODS, y = AVERAGE.GROSS) +
  geom_col(fill = "#112446") +
  labs(title = "Production Method vs. Average Gross") +
  theme_minimal()
```

## Analysis 

This is the data visualization of the relationship between market response and production methods, such as live action, digital animation and stop-motion animation. We choose the number of movies, average gross and market share as the main analysis factors. According to the stats table, there is a big gap between the maximum and minimum values of these three factors, especially the market share, the largest value is 75.56 percent while the smallest value is close to 0 percent. These two graphs above can show a clear image of the differences in movie quantities and average gross. The first graph is about movie quantities, we can see movies depends on live action has a very prominent number, but according to the second graph, which is about average gross of each production method, the average gross of live action does not have an outstanding performance. Instead, the production method that has the highest average gross is the combination of animation and live action. It seems that there are a lot of pure live action movies, but not all of them are of excellent quality. At the same time, movies that combine animation and live action have a good market response. Perhaps film companies can consider producing more such movies.

# Conclusion

According to our EDA we made a bold assumption that adventure produced by Disney World with production method of animation and live action will make most profits money. But as a rigorous statistical scientist, we can't make judgments and predictions just based on charts and graphs, it's very uncritical. Hence, if more accurate predictions are needed, we need to build specific models to check which attribute is highly impact with the movie make money. However, due to the limit of knowledge and data support could have to stop at this point. 
